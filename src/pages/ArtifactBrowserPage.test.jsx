...
describe('ArtifactBrowserPage', () => {
  ...
  it('renders a list of artifacts when fetched successfully', async () => {
    const artifacts = [
      ...
      makeArtifact({ id: 'a2', artifact_type: 'outreach_draft', content: { email_draft: 'Hi' } }),
    ]
  ...
  it('shows empty state when no artifacts exist', async () => {
    ...
    expect(screen.getByRole('status', { name: /no artifacts found/i })).toBeInTheDocument()
  })
  ...
  it('filters artifacts by type when a filter checkbox is toggled', async () => {
    const allArtifacts = [
      makeArtifact({ id: 'a1', artifact_type: 'audit_report' }),
      makeArtifact({ id: 'a2', artifact_type: 'plan', content: { text: 'A plan' } }),
    ]
    ...
    await waitFor(() => {
      expect(getArtifacts).toHaveBeenCalledWith({ artifact_type: 'audit_report' })
    })
  })
  ...
  it('toggles ArtifactViewer off when clicking the same artifact again', async () => {
    ...
    // Close
    fireEvent.click(item)
    expect(screen.queryByRole('region', { name: /artifact viewer/i })).not.toBeInTheDocument()
  })

  it('uses client-side filtering when multiple types are selected', async () => {
    const allArtifacts = [
      makeArtifact({ id: 'a1', artifact_type: 'audit_report' }),
      makeArtifact({ id: 'a2', artifact_type: 'plan', content: { text: 'Plan' } }),
      makeArtifact({ id: 'a3', artifact_type: 'code_snippet', content: { code: '...' } }),
    ]
  ...
  it('renders filter controls for all six artifact types', async () => {
    ...
    expect(screen.getByLabelText('Filter by Outreach Draft')).toBeInTheDocument()
    expect(screen.getByLabelText('Filter by Workflow Definition')).toBeInTheDocument()
    ...
    expect(screen.getByLabelText('Filter by Research Summary')).toBeInTheDocument()
  })
