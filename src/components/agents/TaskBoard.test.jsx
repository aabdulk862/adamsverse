...
const { mockSubscribe, mockOn, mockRemoveChannel, mockChannel, mockGetTasksByPipeline } = vi.hoisted(() => {
  ...
  const mockGetTasksByPipeline = vi.fn().mockResolvedValue({ data: null, error: null })
  return { mockSubscribe, mockOn, mockRemoveChannel, mockChannel, mockGetTasksByPipeline }
})
...
describe('TaskBoard', () => {
  ...
  it('shows all tasks completed in progress when all are completed', () => {
    const tasks = [
      { id: 't1', pipeline_id: 'p1', name: 'Task A', agent_role_name: 'Planner', agent_role_id: 'r1', status: 'completed' },
      { id: 't2', pipeline_id: 'p1', name: 'Task B', agent_role_name: 'Research', agent_role_id: 'r2', status: 'completed' },
    ]
  ...
  describe('polling fallback', () => {
    ...
    it('cleans up polling interval on unmount', () => {
      ...
      const tasks = makeTasks()
      const { unmount } = render(<TaskBoard tasks={tasks} pipelineStatus="in_progress" />)
