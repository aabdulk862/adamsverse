...
export default function ProjectTimeline({ history = [] }) {
  ...
  return (
    <div className="project-timeline" role="list" aria-label="Project status history">
      {history.map((entry) => {
        const dotColor = STATUS_COLORS[entry.status] || '#6b7280';
        const statusClass = statusToClass(entry.status);

        return (
          <div className="project-timeline-entry" key={entry.id} role="listitem">
            ...
            <div className="project-timeline-content">
              <span className={`project-timeline-status dashboard-project-status dashboard-project-status--${statusClass}`}>
                {entry.status}
              </span>
              {entry.description && (
                <p className="project-timeline-description">{entry.description}</p>
              )}
              <time className="project-timeline-date" dateTime={entry.created_at}>
