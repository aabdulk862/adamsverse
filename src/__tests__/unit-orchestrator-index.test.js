...
const mockRoles = {
  planner: { id: 'role-planner-uuid', name: 'Planner', role_type: 'planner', status: 'active' },
  research: { id: 'role-research-uuid', name: 'Research', role_type: 'research', status: 'active' },
  builder: { id: 'role-builder-uuid', name: 'Builder', role_type: 'builder', status: 'active' },
  audit: { id: 'role-audit-uuid', name: 'Audit', role_type: 'audit', status: 'active' },
  automation: { id: 'role-automation-uuid', name: 'Automation', role_type: 'automation', status: 'active' }
}
...
describe('classifyIntent', () => {
  ...
  it('picks the intent with the most keyword matches', async () => {
    ...
    const result = await classifyIntent('build and create some code with a plan')
    expect(result.error).toBeNull()
    expect(result.data).toBe('build')
  })
})
...
describe('decomposeRequest', () => {
  ...
  it('decomposes a "research" request into tasks', async () => {
    const result = await decomposeRequest('research competitor pricing', 'research')
    expect(result.error).toBeNull()
    expect(result.data.length).toBeGreaterThanOrEqual(1)
  })
...
describe('synthesizeResults', () => {
  ...
  it('returns error when tasks fetch fails', async () => {
    getPipelineRun.mockResolvedValue({
      data: { id: 'pipeline-123', request_summary: 'test', status: 'completed', created_at: '2024-01-01' },
      error: null
    })
    ...
    const mockTasks = [
      {
        ...
        output_data: { plan: [{ step: 1, description: 'Design layout' }], summary: 'Plan ready' }
      },
      {
        id: 'task-2',
  ...
  it('handles tasks with no output_data', async () => {
    getPipelineRun.mockResolvedValue({
      data: { id: 'p-1', request_summary: 'test', status: 'completed', created_at: '2024-01-01' },
      error: null
    })
    getTasksByPipeline.mockResolvedValue({
      data: [
        { id: 't-1', name: 'Task 1', status: 'completed', agent_role_id: 'r-1', output_data: null },
        { id: 't-2', name: 'Task 2', status: 'failed', agent_role_id: 'r-2', output_data: null }
      ],
      error: null
    })
