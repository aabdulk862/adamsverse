...
const mockRole = {
  ...
  output_schema: { type: 'object', properties: { plan: { type: 'array' }, summary: { type: 'string' } }, required: ['plan', 'summary'] }
}
...
describe('createPipeline', () => {
  ...
  it('creates a pipeline run and tasks successfully', async () => {
    const pipelineRun = { id: 'pipeline-1', request_summary: 'test', status: 'pending' }
...
describe('executePipeline', () => {
  ...
  it('detects cyclic dependencies and fails the pipeline', async () => {
    ...
    expect(updatePipelineRun).toHaveBeenCalledWith('pipeline-1', { status: 'failed' })
  })
  ...
  it('executes tasks in dependency order and completes pipeline', async () => {
        ...
        data: [
          {
            ...
            input_data: { goal: 'test' },
            output_data: taskStatuses['task-1'] === 'completed' ? { plan: [] } : null,
            depends_on: []
          },
          {
            id: 'task-2',
            ...
            input_data: { query: 'test' },
            output_data: taskStatuses['task-2'] === 'completed' ? { findings: [] } : null,
            depends_on: ['task-1']
          }
        ],
        error: null
    ...
    expect(updatePipelineRun).toHaveBeenCalledWith(pipelineId, { status: 'in_progress' })
  ...
  it('marks task and dependents as failed on task failure', async () => {
    ...
    expect(updateTask).toHaveBeenCalledWith('task-1', expect.objectContaining({ status: 'failed' }))
    ...
    // Pipeline should be marked as failed
    expect(updatePipelineRun).toHaveBeenCalledWith(pipelineId, { status: 'failed' })
  })
})
