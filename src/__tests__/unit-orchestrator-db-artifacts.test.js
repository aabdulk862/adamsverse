...
describe('createArtifact', () => {
  ...
  it('validates content against agent role output_schema before storing', async () => {
    ...
    setupCreateArtifactMocks(
      ...
      { data: { id: 'artifact-1', agent_role_id: 'role-1', content: { summary: 'Test' } }, error: null }
    )
  ...
  it('serializes content through JSON round-trip before storing', async () => {
    ...
    setupCreateArtifactMocks(
      { data: { id: 'role-1', output_schema: { type: 'object' } }, error: null },
...
describe('getArtifacts', () => {
  ...
  it('filters by artifact_type when provided', async () => {
    const artifacts = [
      { id: 'a1', content: { summary: 'Audit' }, artifact_type: 'audit_report' }
    ]
    ...
    expect(callLog.eq.some(args => args[0] === 'artifact_type' && args[1] === 'audit_report')).toBe(true)
  ...
  it('sorts by created_at descending', async () => {
    ...
    expect(callLog.order.some(args => args[0] === 'created_at' && args[1]?.ascending === false)).toBe(true)
  })
  ...
  it('returns Supabase errors when query fails', async () => {
    queryResult = { data: null, error: { message: 'DB error', code: 'DB_ERR' } }
...
describe('getArtifactById', () => {
  ...
  it('deserializes content for the retrieved artifact', async () => {
    ...
    mockFrom.mockImplementation((...args) => {
      callLog.from.push(args)
      const chain = createChain()
      chain.single = vi.fn(() => Promise.resolve({ data: artifact, error: null }))
      return chain
    })
